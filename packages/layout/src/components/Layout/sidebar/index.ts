...
export class MagicLayoutSidebar extends MagicElement<
	RequiredMagicLayoutOptions['sidebar']
> {
	...
	_addSidebarCollapsedEvent() {
		this.store.root.addEventListener(
			'sidebar-collapsed',
			this._onSidebarCollapsed.bind(this),
			true,
		); // 使用捕获阶段监听
	}
	...
	getSidebarWidth() {
		return getCssSize(
			this.state.collapsed ? this.state.collapsedWidth : this.state.width,
		);
	}
	render() {
		this.classs.use({
